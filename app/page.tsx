...
function Section({
  ...
  return (
    <section className="grid grid-cols-1 gap-y-5 border-t border-border pt-7 md:grid-cols-[8.5rem_1fr] md:gap-x-12 print:grid-cols-[7.5rem_1fr] print:gap-x-8 print:pt-5">
      <h2 className="text-[11px] font-medium tracking-[0.22em] text-muted-foreground uppercase md:mt-1 print:break-after-avoid">
        {label}
      </h2>
  ...
  return (
    <div className="space-y-4 print:space-y-2.5">
      {groups.map((group) => (
        <div
          key={group.name}
          className="grid break-inside-avoid gap-x-6 gap-y-1 sm:grid-cols-[10rem_1fr] print:grid-cols-[9rem_1fr]"
        >
